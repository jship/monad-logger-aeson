...
module TestCase.LogWarnN.MetadataNoThreadContextYes
  ( testCase
  ) where

import Control.Monad.Logger.Aeson
  ( (.=), Loc(..), LogLevel(..), LoggedMessage(..), logWarnN, withThreadContext
  )
        ...
        [aesonQQ|
          {
            ...
            "location": {
              "package": "main",
              "module": "TestCase.LogWarnN.MetadataNoThreadContextYes",
              "file": "test-suite/TestCase/LogWarnN/MetadataNoThreadContextYes.hs",
              "line": 22,
              "char": 11
            },
            "context": {
    ...
    , expectedLoggedMessage =
        LoggedMessage
          ...
          , loggedMessageLoc =
              Just Loc
                { loc_package = "main"
                , loc_module = "TestCase.LogWarnN.MetadataNoThreadContextYes"
                , loc_filename = "test-suite/TestCase/LogWarnN/MetadataNoThreadContextYes.hs"
                , loc_start = (22, 11)
                , loc_end = (0, 0)
                }
