...
module TestCase.LogInfo.MetadataYesThreadContextYes
  ( testCase
  ) where

import Control.Monad.Logger.Aeson
  ( (.=), Loc(..), LogLevel(..), LoggedMessage(..), Message(..), logInfo, withThreadContext
  )
        ...
        [aesonQQ|
          {
            ...
            "location": {
              "package": "main",
              "module": "TestCase.LogInfo.MetadataYesThreadContextYes",
              "file": "test-suite/TestCase/LogInfo/MetadataYesThreadContextYes.hs",
              "line": 23,
              "char": 11
            },
            "context": {
    ...
    , expectedLoggedMessage =
        LoggedMessage
          ...
          , loggedMessageLoc =
              Just Loc
                { loc_package = "main"
                , loc_module = "TestCase.LogInfo.MetadataYesThreadContextYes"
                , loc_filename = "test-suite/TestCase/LogInfo/MetadataYesThreadContextYes.hs"
                , loc_start = (23, 11)
                , loc_end = (0, 0)
                }
