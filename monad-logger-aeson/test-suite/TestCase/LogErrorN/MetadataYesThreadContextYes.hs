...
module TestCase.LogErrorN.MetadataYesThreadContextYes
  ( testCase
  ) where

import Control.Monad.Logger.Aeson
  ( (.=), Loc(..), LogLevel(..), LoggedMessage(..), Message(..), logErrorN, withThreadContext
  )
        ...
        [aesonQQ|
          {
            ...
            "location": {
              "package": "main",
              "module": "TestCase.LogErrorN.MetadataYesThreadContextYes",
              "file": "test-suite/TestCase/LogErrorN/MetadataYesThreadContextYes.hs",
              "line": 22,
              "char": 11
            },
            "context": {
    ...
    , expectedLoggedMessage =
        LoggedMessage
          ...
          , loggedMessageLoc =
              Just Loc
                { loc_package = "main"
                , loc_module = "TestCase.LogErrorN.MetadataYesThreadContextYes"
                , loc_filename = "test-suite/TestCase/LogErrorN/MetadataYesThreadContextYes.hs"
                , loc_start = (22, 11)
                , loc_end = (0, 0)
                }
