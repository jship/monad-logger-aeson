...
module TestCase.LogErrorN.MetadataNoThreadContextYes
  ( testCase
  ) where

import Control.Monad.Logger.Aeson
  ( (.=), Loc(..), LogLevel(..), LoggedMessage(..), logErrorN, withThreadContext
  )
        ...
        [aesonQQ|
          {
            ...
            "location": {
              "package": "main",
              "module": "TestCase.LogErrorN.MetadataNoThreadContextYes",
              "file": "test-suite/TestCase/LogErrorN/MetadataNoThreadContextYes.hs",
              "line": 22,
              "char": 11
            },
            "context": {
    ...
    , expectedLoggedMessage =
        LoggedMessage
          ...
          , loggedMessageLoc =
              Just Loc
                { loc_package = "main"
                , loc_module = "TestCase.LogErrorN.MetadataNoThreadContextYes"
                , loc_filename = "test-suite/TestCase/LogErrorN/MetadataNoThreadContextYes.hs"
                , loc_start = (22, 11)
                , loc_end = (0, 0)
                }
